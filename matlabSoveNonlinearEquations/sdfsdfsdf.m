...
FileName = {'building_cross';'kitchen_cross'; 'rnl_cross'; 'campus_cross';'galileo_cross';'stpeters_cross';'uffizi_cross';'grace_cross' };
...
TFileName = 'chicNN_FeatureToLight_8_4025.obj';


initInput('all', TFileName, LFileName)
isScaled = false;
LightABOpt = optimLight();
saveLight(LightABOpt, 'all');

for i=1:8
    initInput(char(FileName(i)), TFileName, LFileName)
    LightABOpt = optimLight();
    saveLight(LightABOpt, char(FileName(i)));
end;


%%
 function [] = initInput(FileName, TFileName, LFileName)

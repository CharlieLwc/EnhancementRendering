...
if size(barvalues,1) ~= size(errors,1) || size(barvalues,2) ~= size(errors,2)
	...
	if ~isempty(bw_legend) && ~strcmp(legend_type, 'axis')
		handles.legend = legend(bw_legend, 'location', 'best', 'fontsize',22);
		legend boxoff;
	else
		handles.legend = [];
    end
	...
	if strcmp(legend_type, 'axis')
		for i = 1:numbars
			...
			for j = 1:length(xdata)
				text(xdata(j),  -0.03*ymax*1.1, bw_legend(i), 'Rotation', 60, 'fontsize', 22, 'HorizontalAlignment', 'right');
			end
		end
	...
	if ~isempty(bw_title)
		title(bw_title, 'fontsize',24);
	end
	if ~isempty(bw_xlabel)
		xlabel(bw_xlabel, 'fontsize',24);
	end
	if ~isempty(bw_ylabel)
		ylabel(bw_ylabel, 'fontsize',24);
	end
	
	set(gca, 'xticklabel', groupnames, 'box', 'off', 'ticklength', [0 0], 'fontsize', 22, 'xtick',1:numgroups, 'linewidth', 2,'xgrid','off','ygrid','off');

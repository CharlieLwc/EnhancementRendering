 ...
 *
 * $Id: pfs.cpp,v 1.13 2014/09/17 15:07:18 rafm Exp $
 */
...
static void readTags( TagContainerImpl *tags, FILE *in )
{
  int readItems;
  int tagCount;
  readItems = fscanf_s( in, "%d" PFSEOL, &tagCount );
...
public:
  ...
  Frame *readFrame( FILE *inputStream )
  {
    ...
    int old_mode = _setmode( _fileno( inputStream ), _O_BINARY );
#endif
    ...
    read = fscanf_s( inputStream, "%d %d" PFSEOL, &width, &height );
	...
	read = fscanf_s(inputStream, "%d" PFSEOL, &channelCount);
...
#ifdef HAVE_SETMODE
    _setmode( _fileno( inputStream ), old_mode );
#endif
    return frame;
  }
...
delete lastFrame;*/
  ...
  void writeFrame( Frame *frame, FILE *outputStream )
  {
...
#ifdef HAVE_SETMODE
    // Needed under MS windows (text translation IO for stdin/out)
    int old_mode = _setmode( _fileno( outputStream ), _O_BINARY );
#endif
    ...
    fflush(outputStream);
#ifdef HAVE_SETMODE
    _setmode( _fileno( outputStream ), old_mode );
#endif
  }

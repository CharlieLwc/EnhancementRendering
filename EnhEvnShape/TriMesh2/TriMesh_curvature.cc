...
void TriMesh::need_curvatures()
{
	...
	FILE *fin;
	fopen_s(&fin, dFName.getCurvatureDN(), "rb");
	if (fin)
	{
		...
		fread(&averageCurv1, sizeof(float), 1, fin);
		if (totalNum == nv)
		{
			curv1.resize(nv); curv2.resize(nv); pdir1.resize(nv); pdir2.resize(nv);

			fread(&curv1[0], sizeof(float), totalNum, fin);
			fread(&curv2[0], sizeof(float), totalNum, fin);
			fread(pdir1[0], sizeof(float), totalNum * 3, fin);
			fread(pdir2[0], sizeof(float), totalNum * 3, fin);
	...
	/*
	ifstream curvature_if(dFName.getCurvatureDN());
	if (curvature_if.is_open() && curv1.empty())
	{
		cout<<"Reading curvature.....";
		int totalNum;
		curvature_if>>totalNum;
		if (totalNum == nv)
		{
			curv1.resize(nv); curv2.resize(nv); pdir1.resize(nv); pdir2.resize(nv);
			for(int i = 0; i<totalNum; i++)
			{
				curvature_if>>curv1[i];
				curvature_if>>curv2[i];
				curvature_if>>pdir1[i];
				curvature_if>>pdir2[i];
			}
			cout<<"Done"<<endl;
			curvature_if.close();
			return;
...
#pragma omp parallel for
	...
	cout<<"Writing Curvature.....";


	FILE *fout;
	fopen_s(&fout, dFName.getCurvatureDN(), "wb");

	fwrite(&nv, sizeof(int), 1, fout);
	fwrite(&averageCurv1, sizeof(float), 1, fout);
	fwrite(&curv1[0], sizeof(float), nv, fout);
	fwrite(&curv2[0], sizeof(float), nv, fout);
	fwrite(pdir1[0], sizeof(float), nv * 3, fout);
	fwrite(pdir2[0], sizeof(float), nv * 3, fout);

	fclose(fout);
	

	/*
	ofstream curvature_of(dFName.getCurvatureDN());
	curvature_of<<nv<<endl;
	for (int j=0; j<nv; j++)
		curvature_of<<curv1[j]<<" "<<curv2[j]<<" "<<pdir1[j]<<" "<<pdir2[j]<<endl;
	curvature_of.close();
	*/
...
void TriMesh::tune_curve_direction()
{
	...
	if (tune_pdir1)
		...
		tune_pdir1 = true;
	vec LightDirections[6] = {
		vec(0.f, 0.f, -1.f),	//R 1 0 0 

		vec(0.f, 0.f, -1.f),	//L -1 0 0 

		vec(1.f, 0.f, 0.f),	//T 0 1 0	

		vec(1.f, 0.f, 0.f),	//D 0 -1 0 

		vec(0.f, 1.f, 0.f), 	//F 0 0 1

		vec(0.f, -1.f, 0.f)		//B 0 0 -1
	};

	for (unsigned int verIndex = 0; verIndex < vertices.size(); verIndex++)
	{
		vec direction = vertices[verIndex] - bsphere.center;

		normalize(direction);

		vec direction2(0.f, 0.f, 0.f);

		direction2 = vec(abs(direction[1]), direction[2], -abs(direction[0]));

		direction2 = direction2 - direction2*abs(direction2.dot(direction));
		direction2 = normalize(direction2);
		if (pdir1[verIndex].dot(direction2) < 0.f)
			pdir1[verIndex] = -pdir1[verIndex];
...
void TriMesh::need_dcurv()
{
	...
	FILE *fin;
	fopen_s(&fin, dFName.getDcurvatureDN(), "rb");
	if (fin)
	{
		cout << "Reading dcurvature.....";
		int totalNum;
		...
		if (totalNum == nv)
		{
			...
			fread(dcurv[0], sizeof(float), totalNum * 4, fin);
	...
	/*
	ifstream dcurvature_if(dFName.getDcurvatureDN());
	if (dcurvature_if.is_open() && dcurv.empty())
	{
		cout<<"Reading dcurvature.....";
		int totalNum;
		dcurvature_if>>totalNum;
		if (totalNum == nv)
		{
			dcurv.resize(nv);
			for(int i = 0; i<totalNum; i++)
				dcurvature_if>>dcurv[i];
			cout<<"Done"<<endl;
			dcurvature_if.close();
			return;
		}
...
#pragma omp parallel for
	...
	cout << "Writing Dcurvature.....";


	FILE *fout;
	fopen_s(&fout, dFName.getDcurvatureDN(), "wb");

	fwrite(&nv, sizeof(int), 1, fout);
	fwrite(dcurv[0], sizeof(float), nv*4, fout);

	fclose(fout);

	/*
	ofstream dcurvature_of(dFName.getDcurvatureDN());
	dcurvature_of<<nv<<endl;
	for (int j=0; j<nv; j++)
		dcurvature_of<<dcurv[j]<<endl;
	dcurvature_of.close();
	*/

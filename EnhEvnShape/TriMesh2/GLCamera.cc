...
void GLCamera::rotate(int mousex, int mousey, xform &xf)
{
	...
	if (constraint_ == XCONSTRAINED) 
		pos1[1] = pos2[1] = 0.0f;
	if (constraint_ == YCONSTRAINED) 
		pos1[0] = pos2[0] = 0.0f;
	if (constraint_ == ZCONSTRAINED)
		pos1[2] = pos2[2] = 0.0f;
...
void GLCamera::mouse(int mousex, int mousey, Mouse::button b,
		     const point &scene_center, float scene_size,
		     xform &xf, int& rightXInc, int& rightYInc)
{
	...
	{
		rightXInc = (float)mousex - lastmousex;
		rightYInc = (float)mousey - lastmousey;
...
void GLCamera::setupGL(const point &scene_center, float scene_size)
{
	GLint V[4];
	glGetIntegerv(GL_VIEWPORT, V);
	...
	fardist  = max(-(scene_center[2] - 3.f*scene_size),
			     scene_size / DOF);

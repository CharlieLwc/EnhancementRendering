...
void TriMesh::need_tstrips()
{
	...
	FILE *fin;
	fopen_s(&fin, dFName.getTstripsDN(), "rb");
	if (fin)
	{
		cout << "Reading tstrips.....";
		int totalNum;

		fread(&totalNum, sizeof(int), 1, fin);

		tstrips.resize(totalNum);
		fread(&tstrips[0], sizeof(int), totalNum, fin);
	...
	/*
	ifstream tstrips_if(dFName.getTstripsDN());
	if (tstrips_if.is_open())
	{
		cout<<"Reading tstrips.....";
		int totalNum;
		tstrips_if>>totalNum;
		tstrips.reserve(totalNum);
		int temp;
	...
	*/
	...
	cout<<"Writing tstrips.....";


	FILE *fout;
	fopen_s(&fout, dFName.getTstripsDN(), "wb");
	int nv = tstrips.size();
	fwrite(&nv, sizeof(int), 1, fout);
	fwrite(&tstrips[0], sizeof(float), nv, fout);

	fclose(fout);
	/*
	ofstream tstrips_of(dFName.getTstripsDN());
	tstrips_of<<tstrips.size()<<endl;
	for (int j=0; j<tstrips.size(); j++)
		tstrips_of<<tstrips[j]<<" ";
	tstrips_of.close();
	*/
